...
def _curves(JRC0, JCS0, phi_b, L, L0, sigma_max, npts, use_scaling=True):
    ...
    # unscaled
    tau_u = np.array([tau_barton_bandis_mpa(s/1000.0, JRC0, JCS0, phi_b) for s in (sigma*1000.0)])  # hack-free reuse? We'll do direct formula below instead
    # Better: compute directly (avoid per-point loop)
...
def render():
    ...
    with st.sidebar:
        st.subheader("Preset")
        preset = st.selectbox("Velg", list(PRESETS.keys()), index=0)
        if st.button("Bruk preset-verdier"):
            p = PRESETS[preset]
            st.session_state.update(p)

        st.subheader("Input")
        JRC0 = st.slider("JRC0", 0.0, 20.0, float(st.session_state.get("JRC0", 12.0)), 0.1)
        JCS0 = st.slider("JCS0 (MPa)", 20.0, 200.0, float(st.session_state.get("JCS0", 80.0)), 1.0)
        phi_b = st.slider("φb (deg)", 25.0, 40.0, float(st.session_state.get("phi_b", 30.0)), 0.1)

        st.subheader("Skalering")
        L = st.slider("L (m)", 1.0, 20.0, float(st.session_state.get("L", 5.0)), 0.5)
        L0 = st.number_input("L0 (m)", 0.01, 1.0, float(st.session_state.get("L0", 0.10)), 0.01)
        use_scaling = st.checkbox("Bruk skalering", value=True)
        show_both = st.checkbox("Vis både uskalert og skalert", value=True)

        st.subheader("σn")
        sigma_max = st.slider("σn maks (MPa)", 0.1, 5.0, 5.0, 0.1)
        npts = st.slider("Punkter", 50, 500, 200, 10)
    ...
    with t1:
        ...
        fig1.update_layout(title="τ vs σn", xaxis_title="σn (MPa)", yaxis_title="τ (MPa)", height=420, margin=dict(l=10,r=10,t=40,b=10))
        ...
        fig2.update_layout(title="φ_active vs σn", xaxis_title="σn (MPa)", yaxis_title="φ_active (deg)", height=420, margin=dict(l=10,r=10,t=40,b=10))
    ...
    with t2:
        ...
        with left:
            out_choice = st.selectbox("Output", ["τ ved valgt σn", "φ_active ved valgt σn"])
        ...
        for p in vary:
            ...
            lo[p] = base[p]*(1-step_pct/100); hi[p] = base[p]*(1+step_pct/100)
            y_lo = eval_output(lo["JRC0"], lo["JCS0"], lo["phi_b"], lo["L"], lo["L0"])
            y_hi = eval_output(hi["JRC0"], hi["JCS0"], hi["phi_b"], hi["L"], hi["L0"])
            effects.append((p, y_lo, y_hi))
            spider["Parameter"].append(p)
        ...
        fig_t.update_layout(barmode="overlay", xaxis_title="Δ output (fra baseline)", height=440, margin=dict(l=10,r=10,t=30,b=10))
        ...
        fig_s.add_trace(go.Scatter(x=sdf["Parameter"], y=sdf["+%"], mode="lines+markers", name=f"+{step_pct}%"))
        fig_s.update_layout(yaxis_title="% endring", xaxis_title="Parameter", height=380, margin=dict(l=10,r=10,t=30,b=10))
        st.plotly_chart(fig_s, use_container_width=True)

    with t3:
        st.subheader("Monte Carlo")
        c1, c2, c3 = st.columns(3)
        with c1: mc_n = st.slider("N", 200, 20000, 2000, 200)
        with c2: mc_sigma = st.slider("σn (MPa)", 0.05, float(sigma_max), min(1.0, float(sigma_max)), 0.05)
        with c3: mc_out = st.selectbox("Output", ["τ", "φ_active"])
        mc_curve = st.radio("Kurve", ["Skalert","Uskalert"], horizontal=True)

        u1,u2,u3,u4,u5 = st.columns(5)
        with u1: sd_JRC = st.number_input("Std JRC0", 0.0, 10.0, 1.0, 0.1)
        with u2: sd_JCS = st.number_input("Std JCS0 (MPa)", 0.0, 100.0, 10.0, 1.0)
        with u3: sd_phi = st.number_input("Std φb", 0.0, 10.0, 1.0, 0.1)
        with u4: sd_L = st.number_input("Std L", 0.0, 10.0, 1.0, 0.1)
        with u5: sd_L0 = st.number_input("Std L0", 0.0, 0.5, 0.02, 0.01)
        ...
        m,sd = float(out.mean()), float(out.std(ddof=1))
        s1,s2,s3,s4 = st.columns(4)
        s1.metric("Mean", f"{m:.4g}")
        s2.metric("Std", f"{sd:.4g}")
        s3.metric("P50", f"{q50:.4g}")
        s4.metric("P5/P95", f"{q05:.4g}/{q95:.4g}")

        df_mc = pd.DataFrame({"output": out, "JRC0":JRC_samp, "JCS0":JCS_samp, "phi_b":phi_samp, "L":L_samp, "L0":L0_samp})
        ...
        corr = df_mc.corr(numeric_only=True)["output"].drop("output").sort_values(key=lambda s: s.abs(), ascending=False)
        ...
        st.download_button("Last ned Monte Carlo CSV", df_mc.to_csv(index=False).encode("utf-8"),
                           file_name="barton_bandis_monte_carlo.csv", mime="text/csv")
    ...
    with t4:
        ...
        st.download_button("Last ned kurver (CSV)", df.to_csv(index=False).encode("utf-8"),
                           file_name="barton_bandis_curves.csv", mime="text/csv")